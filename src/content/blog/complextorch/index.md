...
description: >
  ...
  model to complex-valued by changing the import — then reach for the parts torch.nn
...
signal before the first layer. A synthetic-aperture-radar pixel, an MR k-space sample, a
received comms symbol — each is natively a **complex number**, and the *phase* carries
structure that the magnitude alone does not: coherence between passes, sub-wavelength
displacement, the geometry of the scene. Stack real and imaginary parts into two channels
and feed them to a standard CNN and you can still learn something, but you've told the
network that real and imaginary are unrelated features rather than two coordinates of one
rotating quantity.
...
There's no bespoke tensor wrapper to learn — the library operates directly on PyTorch's
...
- **Pooling** — `MagMaxPool` (magnitude-argmax, since `>` isn't defined on complex),
  ...
  spectrum (Rippel 2015 / Trabelsi 2018) and preserves the DC bin — and thus the spatial
  mean — exactly.
...
`wFMConvStrict2d` (convex weights, $\sum w = 1$, $w \ge 0$) — compose into equivariant
...
The equivariance and invariance claims aren't asserted — they're checked. Every module in
this family has a correctness test in `tests/invariants/test_equivariance.py`, and the
docs are honest about where the math breaks down (positive convolution padding zero-pads
...
re-runs on every build, so the public-API examples can't silently rot, plus an API
...
complextorch is the open-source foundation under most of the phase-aware work elsewhere on
this site — the same conviction that you get further by respecting the physics of the
signal than by flattening it into generic pixels. See the [Projects](/projects)
page for where it's being put to use.
